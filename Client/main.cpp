...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
	if(fdwReason == DLL_PROCESS_ATTACH)
	{
		...
		{
			MessageBox(0,"Incorrect gta-vc.exe version detected.\nYou must use GTA:VC 1.0 to play Vice City: Multiplayer","Vice City: Multiplayer Error",MB_OK);
		...
		{
			MessageBox(0,"You are missing the vcpfnt.png/dat file from your games directory.","Vice City: Multiplayer Error",MB_OK);
			SetForegroundWindow(HWND_DESKTOP);
			ExitProcess(1);			
		}
...
void InitD3DStuff()
{
	...
	pVersion->setText("Vice City: Multiplayer - Development Version (" __TIME__ ", " __DATE__ ")");
	CEGUI::Font * pFont = pGUI->GetTahomaBoldFont();
	float fTextWidth = pFont->getTextExtent("Vice City: Multiplayer - Development Version (" __TIME__ ", " __DATE__ ")");
...
void logprintf(char * format, ...)
{
	...
	va_end(args);
	puts(tmp_buf);
	FILE* log = fopen("vcmp/client.log", "a");
	fprintf(log, "%s\n", tmp_buf);
	fclose(log);
}
