...
int main (int argc, char* argv[])
{
	...
	logprintf("------------------------------------------");
	logprintf("Vice City:Multiplayer");
	logprintf("2023 RuslanSemchneko");
	logprintf("Donations can help me improve it and upgrade");
	logprintf("PAYEER P1012917935 BTC 36Z2peM1RhZWRovzY6yCMUsPmaz2k6nDfp");
    logprintf("ETH 0x047Bb274cAe2fA0007Fe019D8481bbf77503c31c");
    logprintf("RIPPLE rshvnxLDE9Jsm8sJxPxct425HhQC2tk5CV TAG 10665290");
    logprintf("DASH XuL7kPzgEjP2zi65uiqnkWcHUy6Kgig5jB");
    logprintf("DOGE DSHQuEtYAZKTdJaiRi1YpamaxpFaEST1Mh");
    logprintf("WEB MONEY Z355707713582");
...
bool ProcessServerQuery(SOCKET s, SystemAddress systemAddress, char * szQuery, int iQueryLength)
{
	...
	if(!(iQueryLength > 3 && szQuery[0] == 'V' && szQuery[1] == 'C' && szQuery[2] == 'M'&& szQuery[3] == 'P')) // IVMP
	{
		return false;
	}
#ifdef WIN32
...
void HandleServerQuery(const QueryJob &job)
{
	...
	if(true)
	{
		...
		const int iDataLength = ((sizeof("VCMP") - 1) + sizeof(unsigned char));
