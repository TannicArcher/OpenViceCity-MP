            ...
            if ( hProcess )
            {
                ...
                if ( EnumProcessModules ( hProcess, &pModule, sizeof ( HMODULE ), &cbNeeded ) )
                {
                    ...
                    if ( GetModuleFileNameEx( hProcess, pModule, szModuleName, 500 ) )
                    {
                        ...
                        {
							if ( MessageBox ( 0, "An instance of GTA: Vice City is already running. It needs to be terminated before Vice City Multiplayer can be started. Do you want to do that now?", "Error", MB_YESNO | MB_ICONQUESTION ) == IDYES )
                            {
...
BEGIN_MESSAGE_MAP(CLauncherDlg, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_EN_CHANGE(IDC_SERVER_IP, &CLauncherDlg::OnEnChangeServerIp)
END_MESSAGE_MAP()
...
BOOL CLauncherDlg::OnInitDialog()
{
	...
	RegOpenKeyEx(HKEY_LOCAL_MACHINE,"Software\\VICEMP",0,KEY_READ,&hKey);
	...
	SetDlgItemText(IDC_SERVER_CONFIG,"vcmp//server.ini");
...
void CLauncherDlg::OnLaunch() 
{
	...
	HKEY hKey = 0;
	RegOpenKeyEx(HKEY_LOCAL_MACHINE,"Software\\VICEMP",0,KEY_WRITE,&hKey);

	// If the key doesn't exist create it
	if(!hKey) {
		RegCreateKey(HKEY_LOCAL_MACHINE,"Software\\VICEMP",&hKey);
	}
	...
	if(!CreateProcess(szGtaExe, szParams, NULL, NULL, TRUE, CREATE_SUSPENDED, NULL, GetAppPath(), &siStartupInfo, 
		&piProcessInfo)) {
		MessageBox("Couldn't launch gta-vc.exe.\nDid you install Vice City: Multiplayer to your Vice City directory?");
		return;
	}

	if ( !TerminateGTAIfRunning () )
    {
		MessageBox ("Vice City Multiplayer could not start because an another instance of GTA: Vice City is running.", "Error", MB_ICONERROR );
        return;
    }
...
{
	ShellExecute(m_hWnd, "open","vcmp//vcmp.txt", NULL, NULL, SW_SHOWNORMAL);	
}
...
void CLauncherDlg::OnStartServer()
{
	...
	ShellExecute(m_hWnd, "open","vcmp//server.exe", szServerConfig, NULL, SW_SHOWNORMAL);
}
...
void CLauncherDlg::OnEnChangeServerIp()
{
	// TODO:  If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
